...
module.exports = {
  name: 'degrade',
  description: 'Degrade a staff member to a lower role',
  usage: '%degrade @user OldRole | NewRole',
  category: 'staff',
  adminOnly: true,

  slashData: new SlashCommandBuilder()
    .setName('degrade')
    .setDescription('Degrade a staff member to a lower role')
    .addUserOption(opt => opt.setName('user').setDescription('The user to degrade').setRequired(true))
    .addStringOption(opt => opt.setName('oldrole').setDescription('Their current role to remove').setRequired(true))
    .addStringOption(opt => opt.setName('newrole').setDescription('The new lower role to give').setRequired(true)),

  async execute(message, args) {
    const member = message.mentions.members.first();
    if (!member) return message.reply('❌ Please mention a user! Usage: `%degrade @user OldRole | NewRole`');

    const rest = args.slice(1).join(' ');
    const parts = rest.split('|');
    if (parts.length < 2) return message.reply('❌ Usage: `%degrade @user OldRole | NewRole`');
    ...
    try {
      await member.send(`📢 You have been **degraded** from **${oldRole.name}** to **${newRole.name}** in **${message.guild.name}**.`);
    } catch {}

    const embed = new EmbedBuilder()
      .setColor(0xe67e22)
      .setTitle('⬇️ Staff Degraded')
      .setDescription(`${member.user.tag} has been degraded.`)
      .setThumbnail(member.user.displayAvatarURL())
      .addFields(
        { name: 'User', value: `${member}`, inline: true },
        { name: 'Old Role', value: `${oldRole}`, inline: true },
        { name: 'New Role', value: `${newRole}`, inline: true },
        { name: 'Degraded By', value: `${message.author}`, inline: false }
      )
      .setTimestamp();
  ...
  async executeSlash(interaction) {
    ...
    try {
      await member.send(`📢 You have been **degraded** from **${oldRole.name}** to **${newRole.name}** in **${interaction.guild.name}**.`);
    } catch {}
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Old Role', value: `${oldRole}`, inline: true },
        { name: 'New Role', value: `${newRole}`, inline: true },
        { name: 'Degraded By', value: `${interaction.user}`, inline: false }
      )
      .setTimestamp();
